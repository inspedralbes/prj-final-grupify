  ...
  <div class="min-h-screen bg-gradient-to-b from-sky-50 to-white">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="text-center mb-12 transform transition-all duration-500 hover:scale-105">
        <div class="inline-block bg-white rounded-2xl px-8 py-4 shadow-xl mb-6 bg-opacity-90 backdrop-blur-sm">
          <h1 class="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-[#00ADEC] to-[#0080C0]">
            RESULTATS SOCIOGRAMA
          </h1>
        </div>
        <div class="h-1.5 w-48 bg-gradient-to-r from-[#00ADEC] to-[#0080C0] mx-auto rounded-full"></div>
      </div>
      ...
      >
        <div class="relative w-20 h-20">
          <div class="absolute inset-0 rounded-full border-4 border-[#00ADEC] border-t-transparent animate-spin"></div>
          <div class="absolute inset-2 rounded-full border-4 border-[#0080C0] border-t-transparent animate-spin-slow"></div>
        </div>
        <p class="mt-6 text-xl text-gray-600 font-medium tracking-wide animate-pulse">
      ...
      >
        <svg class="w-16 h-16 text-red-500 mr-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-else class="space-y-8">
        ...
        <div class="bg-white rounded-3xl shadow-xl p-8 transform transition-all duration-500 hover:shadow-2xl backdrop-blur-sm bg-opacity-90">
          <div class="flex flex-col space-y-8">
            ...
            <div class="grid md:grid-cols-2 gap-8">
              <div class="bg-gradient-to-br from-[#00ADEC]/10 to-[#0080C0]/10 p-8 rounded-2xl border-2 border-[#00ADEC]/20 hover:shadow-xl transition-all duration-300 group">
                <h2 class="text-sm font-semibold text-[#00ADEC] uppercase tracking-wider mb-3 group-hover:translate-x-1 transition-transform">
                  Curs
                </h2>
                <p class="text-4xl font-bold text-gray-800 flex items-center group-hover:translate-x-2 transition-transform">
                  <span class="mr-4 text-[#00ADEC] transform transition-transform group-hover:rotate-12">
        ...
        <div class="flex justify-center space-x-6">
          <button
            v-for="(label, key) in { relations: 'Relacions', roles: 'Rols', skills: 'Competencies' }"
            :key="key"
            ...
            :class="[
              ...
              'overflow-hidden',
              activeComponent === key ? 'ring-4 ring-offset-2 ring-[#00ADEC]/50' : '',
              key === 'relations' ? 'bg-[#0080C0]' : key === 'roles' ? 'bg-[#009FD4]' : 'bg-[#00BFFF]'
            ]"
          >
        ...
        <div class="mt-8">
          <TransitionGroup name="fade" mode="out-in">
            <div v-if="activeComponent === 'relations'" key="relations" class="animate-fade-in">
              <Relations :relationships="filteredRelationships" />
            </div>
            <div v-else-if="activeComponent === 'roles'" key="roles" class="animate-fade-in">
              <Roles />
            </div>
            <div v-else-if="activeComponent === 'skills'" key="skills" class="animate-fade-in">
              <Skills />
            </div>
          </TransitionGroup>
        </div>
