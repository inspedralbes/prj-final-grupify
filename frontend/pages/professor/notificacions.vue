...
const startScheduleChecker = () => {
  setInterval(() => {
    ...
    scheduledNotifications.value = scheduledNotifications.value.filter(notification => {
    ...
    localStorage.setItem('scheduledNotifications', JSON.stringify(scheduledNotifications.value));
  }, 60000);
};
...
const saveNotifications = () => {
  localStorage.setItem('professorNotifications', JSON.stringify(sentNotifications.value));
};
...
const confirmSend = async () => {
  ...
  if (notificationData.scheduledAt) {
    scheduledNotifications.value = [notificationData, ...scheduledNotifications.value];
    localStorage.setItem('scheduledNotifications', JSON.stringify(scheduledNotifications.value));
...
const deleteScheduledNotification = (notificationId) => {
  sentNotifications.value = sentNotifications.value.filter(n => n.id !== notificationId);
  ...
  <div class="notification-panel max-w-2xl mx-auto">
    <div class="bg-white p-6 rounded-lg shadow-md relative">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div>
          <label class="block text-sm font-medium mb-2">Programar notificació (opcional)</label>
          <input
      ...
      >
        <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
          ...
          <p class="mb-2"><strong>Prioritat:</strong> {{ priorityOptions.find(o => o.value === pendingNotification.priority)?.label }}</p>
          <p v-if="pendingNotification.scheduledAt" class="mb-4">
            <strong>Programat per:</strong> {{ formatDate(pendingNotification.scheduledAt) }}
          </p>
          <pre class="bg-gray-100 p-4 rounded mb-6 max-h-60 overflow-auto whitespace-pre-wrap break-words text-sm">
{{ pendingNotification.message }}</pre>
      ...
      >
        <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
          <h3 class="text-lg font-bold mb-4">Eliminar notificació programada?</h3>
          <p class="text-gray-600 mb-6">Aquesta acció no es pot desfer.</p>
          <div class="flex justify-end gap-2">
            <button
      ...
      <div class="mt-8">
        ...
        <div v-if="sentNotifications.length === 0" class="text-center text-gray-500 py-4">
          <div class="space-y-2">
        ...
        <div v-else class="space-y-3">
          ...
          >
            <button 
              ...
              @click="notificationToDelete = notification; showDeleteModal = true"
            ...
            <div class="flex items-center gap-2 mb-2">
              <span 
                :class="[
                  ...
                  priorityOptions.find(o => o.value === notification.priority)?.color
                ]"
              >
                {{ priorityOptions.find(o => o.value === notification.priority)?.label }}
              </span>
              <span 
            ...
            <p class="text-gray-800 whitespace-pre-wrap pr-8">{{ notification.message }}</p>
