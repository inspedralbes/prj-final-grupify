    ...
    <div class="max-w-3xl mx-auto px-4 mt-8">
      ...
      <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
          ...
          <button @click="router.push(`/professor/studentProfile/${studentId}`)"
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
          <div class="space-y-6">
            ...
            <div v-if="questions[currentQuestionIndex].type === 'text'" class="mt-4">
              <input v-model="responses[questions[currentQuestionIndex].id].value" type="text"
            ...
            <div v-if="questions[currentQuestionIndex].type === 'multiple'" class="space-y-3">
              <div v-for="option in questions[currentQuestionIndex].options" :key="option.id" class="relative">
                <label class="block p-4 rounded-lg border-2 transition-all duration-200 cursor-pointer" :class="{
                  ...
                  <input v-model="responses[questions[currentQuestionIndex].id].value" type="radio"
                    :name="'question-' + questions[currentQuestionIndex].id" :value="option.id"
            ...
            <div v-if="questions[currentQuestionIndex].type === 'checkbox'" class="space-y-3">
              <div v-for="option in questions[currentQuestionIndex].options" :key="option.id" class="relative">
                <label class="block p-4 rounded-lg border-2 transition-all duration-200 cursor-pointer" :class="{
                  ...
                  <input v-model="responses[questions[currentQuestionIndex].id].value" type="checkbox"
                    :value="option.id" class="absolute opacity-0" />
                  <span class="text-gray-700">{{ option.text }}</span>
                </label>
              </div>
            </div>
            ...
            <div v-if="questions[currentQuestionIndex].type === 'number'" class="mt-4">
              <input v-model="responses[questions[currentQuestionIndex].id].value" type="number"
                class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200"
            ...
            <div v-if="questions[currentQuestionIndex].type === 'rating'" class="mt-4">
              <div class="grid grid-cols-5 gap-3">
                <label v-for="n in 5" :key="n" class="cursor-pointer text-center">
                  <input type="radio" v-model="responses[questions[currentQuestionIndex].id].value" :value="n"
                    :name="'question-' + questions[currentQuestionIndex].id" class="hidden" />
                  <div class="p-4 rounded-lg transition-all duration-200" :class="{
            ...
            <div class="flex justify-between mt-8 pt-6 border-t">
              ...
              <button v-if="currentQuestionIndex === questions.length - 1" @click="submitResponses"
                class="px-6 py-3 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors duration-200"
                :disabled="isSubmitting" :class="{ 'opacity-70 cursor-wait': isSubmitting }">
      ...
      <div v-else-if="!isLoading && questions.length === 0" class="bg-white rounded-xl shadow-lg p-8 text-center">
        ...
        <p class="text-gray-600 mb-4">Este formulario no tiene preguntas configuradas o no se encontraron las
          competencias
          necesarias.</p>
        <button @click="router.push(`/professor/studentProfile/${studentId}`)"
          class="px-6 py-3 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors duration-200">
