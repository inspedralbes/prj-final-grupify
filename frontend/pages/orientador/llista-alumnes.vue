  ...
  try {
    ...
    if (authStore.user?.course_divisions?.length > 0) {
      ...
      try {
        ...
        const response = await fetch(`https://api.basebrutt.com/api/orientador-assignments/${authStore.user.id}`, {
          headers: {
...
const filterStudentsByClass = async (cls) => {
  try {
    ...
    // Almacenar en localStorage que el orientador quiere ver todos los estudiantes
    localStorage.setItem('orientadorViewAllStudents', cls ? 'false' : 'true');
  } catch (error) {
...
onMounted(async () => {
  ...
  // Verificar preferencia almacenada del orientador
  const viewAllStudents = localStorage.getItem('orientadorViewAllStudents');
  if (viewAllStudents === 'true') {
    // Si la preferencia es ver todos, no hacemos nada adicional ya que ya hemos cargado todos
    console.log("Preferencia guardada: Ver todos los estudiantes");
  }
    ...
    <main class="container mx-auto px-4 py-6 sm:py-8">
      ...
      <div v-else>
        ...
        <div v-if="assignedClasses.length > 0" class="bg-white rounded-lg shadow mb-6">
          ...
          <div class="p-4 sm:p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div @click="filterStudentsByClass(null)"
              ...
              <div v-for="(cls, index) in assignedClasses" :key="index" @click="filterStudentsByClass(cls)"
                class="course-selection-card border rounded-lg p-4 cursor-pointer transition-all duration-300 flex flex-col items-center text-center"
                :class="[
        ...
        <div v-else-if="!isLoading" class="bg-white rounded-lg shadow p-8 text-center">
          ...
          <p class="text-sm text-gray-600">No tens classes assignades. Contacta amb l'administrador perquè t'assigni les
            classes corresponents.</p>
          <div class="mt-4">
            <button @click="fetchAllAssignedStudents"
