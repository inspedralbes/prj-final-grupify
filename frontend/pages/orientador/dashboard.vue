...
const menuItems = [
  ...
  {
    title: "Cesc",
    icon: "M12 9v3.75m0-10.036A11.959 11.959 0 0 1 3.598 6 A11.99 11.99 0 0 0 3 9.75c0 5.592 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.57-.598-3.75h-.152c-3.196 0-6.1-1.25-8.25-3.286Zm0 13.036h.008v.008H12v-.008Z",
...
const navigateToClass = (classItem) => {
  // Establecer la preferencia de ver todos los datos cuando se navega entre secciones
  localStorage.setItem('orientadorViewAllData', 'false');
  localStorage.setItem('orientadorViewAllStudents', 'false');

  // Navegar a la página de la clase específica
  router.push({
    path: `/orientador/clase/${classItem.courseId}-${classItem.divisionId}`
  });
};
          ...
          <div class="flex items-center space-x-4">
            ...
            <div class="flex items-center">
              <div v-if="userData" class="flex items-center">
                <img :src="userData.image || 'https://i.pravatar.cc/150?img=42'" alt="Avatar"
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
              ...
              <div class="flex items-center">
                <!-- Remove the link to see all students -->
              </div>
            </div>
            ...
            <div v-else-if="filteredClasses.length === 0" class="py-10 text-center">
              ...
              <p class="mt-1 text-sm text-gray-500">
                {{ dashboardState.currentClasses.length > 0 ? `No hi ha classes de ${dashboardState.tipoEnsenanzaFilter
                  === 'ESO' ? 'ESO' : 'Batxillerat'} assignades.` : 'Encara no tens cap classe assignada.' }}
              </p>
            </div>

            <!-- Lista de clases -->
            <div v-else class="divide-y divide-gray-200">
              <div v-for="(classItem, index) in filteredClasses" :key="index"
                ...
                <div class="flex items-center">
                  ...
                  <div class="ml-4">
                    ...
                    <div class="flex items-center mt-1">
                      ...
                      <span v-if="classItem.tipoEnsenanza" class="ml-2 text-xs font-medium px-2 py-0.5 rounded-full"
                    ...
                    <div class="flex items-center mt-1">
                      <div class="flex space-x-1">
                        <div class="flex items-center mr-2">
                          ...
                          <span class="text-xs">Social: {{ dashboardState.cescData.social.completed }} / {{
                            dashboardState.cescData.social.total }}</span>
                        </div>
                        <div class="flex items-center mr-2">
                          <div class="w-2 h-2 rounded-full bg-red-500 mr-1"></div>
                          <span class="text-xs">Violent: {{ dashboardState.cescData.violent.completed }} / {{
                            dashboardState.cescData.violent.total }}</span>
                        </div>
                        <div class="flex items-center">
                          <div class="w-2 h-2 rounded-full bg-yellow-500 mr-1"></div>
                          <span class="text-xs">Afectat: {{ dashboardState.cescData.affected.completed }} / {{
                            dashboardState.cescData.affected.total }}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div class="flex items-center mt-1">
                      ...
                      <span class="text-xs text-gray-500 ml-2">
                        ...
                        ({{ dashboardState.sociogramData.total > 0 ?
                          Math.round((dashboardState.sociogramData.completed / dashboardState.sociogramData.total) * 100)
                        : 0 }}%)
                      </span>
                    </div>
                  </div>
