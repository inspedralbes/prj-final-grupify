  ...
  <div class="space-y-8 mt-8">
    ...
    <div class="bg-white rounded-lg shadow p-4 sm:p-6">
      <TeacherStudentFilters v-model:search-query="searchQuery" v-model:selected-course="selectedCourse"
        v-model:selected-division="selectedDivision" @update:selected-course="handleCourseChange"
    ...
    <div v-if="filteredStudents.length > 0" class="bg-white rounded-lg shadow overflow-hidden">
      ...
      <!-- En la sección de la tabla de estudiantes -->
      <div v-if="filteredStudents.length > 0" class="bg-white rounded-lg shadow overflow-hidden">
...
const selectStudent = async (student) => {
  ...
  try {
    // Intentar cargar los datos de competencias del estudiante
    await fetchStudentCompetenciasData(student.id);

    // Verificar si hay datos disponibles
    hasStudentData.value = availableYears.value.length > 0;
  } catch (err) {
...
const fetchStudentCompetenciasData = async (studentId) => {
  // En un entorno real, esta función llamaría a la API
  // Aquí usamos la función de generación de datos aleatorios para simular

  // Simular una probabilidad del 30% de que no haya datos
  const hasData = Math.random() > 0.3;

  if (hasData) {
    generateRandomData();
  } else {
    // Limpiar datos si no hay
    availableYears.value = [];
    competenciasData.value = [];
  }

  // Para implementación real, descomentar este código:
  /*
  const response = await fetch(`http://tu-api.com/competencias/${studentId}/form4`);
...
const courses = ref([]);
const divisions = ref([]);
const selectedInvitationCourse = ref(null);
...
// Funció per obtenir les divisions segons el curs seleccionat per a invitacions
const fetchInvitationDivisions = async () => {
  ...
  try {
    const response = await fetch(
      `https://api.basebrutt.com/api/course-divisions?course_id=${selectedInvitationCourse.value}`,
      {
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
