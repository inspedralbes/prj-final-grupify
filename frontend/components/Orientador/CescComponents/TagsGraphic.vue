  ...
  <div class="space-y-6 mt-2">
    ...
    <div v-else-if="graphData.length > 0" class="bg-white rounded-2xl shadow-xl overflow-hidden">
      ...
      <div class="p-4">
        <div class="flex items-start justify-between mb-3">
          <button @click="handleReturn"
            class="flex items-center py-1 px-4 bg-[#0072AF] hover:bg-[#005d8f] text-white rounded-md transition-colors">
        ...
        <p class="text-gray-600 text-center mb-4">Anàlisi de la distribució de puntuacions segons les categories CESC:
          Popular (A), Rebutjat (C), Agressiu (B), Prosocial (A) i Víctima (C)</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-6">
        ...
        <div v-if="categoria === 'all' || categoria === 'social'"
          class="bg-green-50 rounded-xl p-4 border border-green-100 flex items-start">
          ...
          <div>
            ...
            <p class="text-green-600 mt-1">Estudiants que són populars entre els seus companys. Tenen bona acceptació
              social al grup.</p>
          </div>
        </div>

        <!-- REBUJAT (C) - Solo visible si categoría es 'all' o 'afectado' -->
        <div v-if="categoria === 'all' || categoria === 'afectado'"
          class="bg-blue-50 rounded-xl p-4 border border-blue-100 flex items-start">
          ...
          <div>
            ...
            <p class="text-blue-600 mt-1">Estudiants que són rebutjats pels seus companys. Identificar aquests casos és
              crucial per millorar la integració social.</p>
          </div>
        </div>

        <!-- AGRESSIU (B) - Solo visible si categoría es 'all' o 'violento' -->
        <div v-if="categoria === 'all' || categoria === 'violento'"
          class="bg-red-50 rounded-xl p-4 border border-red-100 flex items-start">
          ...
          <div>
            ...
            <p class="text-red-600 mt-1">Estudiants que mostren comportaments agressius cap a altres. Necessiten atenció
              per millorar la seva interacció social.</p>
          </div>
        </div>

        <!-- PROSOCIAL (A) - Solo visible si categoría es 'all' o 'social' -->
        <div v-if="categoria === 'all' || categoria === 'social'"
          class="bg-purple-50 rounded-xl p-4 border border-purple-100 flex items-start">
          ...
          <div>
            ...
            <p class="text-purple-600 mt-1">Estudiants que mostren comportaments positius i d'ajuda cap als altres. Són
              un actiu valuós per al clima de classe.</p>
          </div>
        </div>

        <!-- VICTIMA (C) - Solo visible si categoría es 'all' o 'afectado' -->
        <div v-if="categoria === 'all' || categoria === 'afectado'"
          class="bg-amber-50 rounded-xl p-4 border border-amber-100 flex items-start">
          ...
          <div>
            ...
            <p class="text-amber-600 mt-1">Estudiants que són víctimes de comportaments negatius. Requereixen atenció i
              suport per prevenir problemes de benestar emocional.</p>
          </div>
        </div>
      </div>
      ...
      <div class="px-6 pb-4">
        <div class="flex flex-wrap gap-4 mb-4">
          <button @click="chartType = 'stacked'" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors"
            :class="chartType === 'stacked' ? 'bg-[#00ADEC] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'">
            Gràfic apilat
          </button>
          <button @click="chartType = 'grouped'" class="px-4 py-2 rounded-lg text-sm font-medium transition-colors"
      ...
      <div class="p-6 pt-0">
        <client-only>
          <VChart class="w-full h-[500px]" :option="chartOptions" autoresize @click="handleChartClick" />
        </client-only>
      </div>
      ...
      <div v-if="selectedClass" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div
            class="bg-gradient-to-r from-[#00ADEC] to-[#0080C0] text-white p-4 rounded-t-xl flex justify-between items-center">
          ...
          <div class="p-6">
            ...
            <div v-if="topStudents.length > 0">
              <h4 class="font-semibold text-gray-800 text-lg mb-4">
                Top 5 Alumnes - {{ getTagNameByIndex(selectedTagIndex) }}
              </h4>
                ...
                <div v-for="(student, index) in topStudents" :key="index"
                  ...
                  <div class="flex items-center">
                    ...
                    <div>
                      ...
                      <p class="text-sm text-gray-500">Participació: {{ student.percentage }}%</p>
                    </div>
                  </div>
                  <span class="font-bold" :class="getTagTextColorClass(selectedTagIndex)">
                    {{ student.points }} punts
                  </span>
                </div>
              </div>

              <div class="mt-4 text-center">
                <p class="text-sm text-gray-600">Aquests alumnes representen el {{ studentsCoverage }}% del total de
                  punts de {{ getTagNameByIndex(selectedTagIndex) }} a la classe.</p>
              </div>
            </div>
            ...
            <div v-else-if="loadingStudents" class="flex flex-col items-center justify-center py-8">
              ...
              <p class="text-gray-600">Carregant dades d'alumnes...</p>
            </div>
            ...
            <div v-else class="text-center py-8">
              ...
              <p class="text-gray-600">No s'han trobat dades d'alumnes per a aquesta categoria.</p>
            </div>
          </div>
    ...
    <div v-else class="text-center bg-white p-8 rounded-xl shadow-md">
      ...
      <h3 class="text-xl font-semibold text-gray-700 mb-2">No hi ha dades disponibles</h3>
      <p class="text-gray-500 max-w-md mx-auto">No s'han trobat dades per mostrar al gràfic. Això pot ser degut a què no
        hi ha resultats CESC registrats o a què no hi ha estudiants amb les etiquetes seleccionades.</p>
    </div>
  </div>
</template>
...
const filteredSeries = computed(() => {
  ...
  const categoryTotals = graphData.value.map(item => {
    ...
    const violentTotal = item.tag_3_count;
    const afectatTotal = item.tag_2_count + item.tag_5_count;
  ...
  if (props.categoria === 'all' || props.categoria === 'violent') {
    series.push({
      name: 'Agressiu (B)',
      type: 'bar',
      stack: chartType.value === 'grouped' ? undefined : 'total',
      ...
      label: {
        ...
        formatter: function (params) {
          ...
          if (chartType.value === 'percentage') {
            // En caso de 'violent', como es el único tag, mostramos el % respecto al total de tags
            if (props.categoria === 'violent') {
  ...
  if (props.categoria === 'all' || props.categoria === 'afectat') {
    // Calculamos el total para la categoría 'afectat' usando la suma de ambos tags
    const afectatTotals = graphData.value.map(item => item.tag_2_count + item.tag_5_count);
    ...
    series.push({
      ...
      data: graphData.value.map((item, index) => {
        if (chartType.value === 'percentage') {
          // Para la categoría 'afectat', calculamos el porcentaje relativo a la suma de ambos tags
          if (props.categoria === 'afectat') {
